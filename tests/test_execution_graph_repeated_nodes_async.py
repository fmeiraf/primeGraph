...
async def test_sequential_repeated_nodes_async():
    ...
    graph.add_repeating_edge(
        "start_task", "repeated_task", END, repeat=3, parallel=False
    )
    graph.compile()
...
async def test_parallel_repeated_nodes_async():
    ...
    graph.add_edge(START, "start_task")
    graph.add_repeating_edge(
        "start_task", "repeated_task", END, repeat=3, parallel=True
    )
    graph.compile()
...
async def test_mixed_repeated_nodes_async():
    ...
    graph.add_edge(START, "start_task")
    graph.add_repeating_edge(
        "start_task", "sequential_task", "intermediate_task", repeat=3, parallel=False
    )
    graph.add_repeating_edge(
        "intermediate_task", "parallel_task", END, repeat=3, parallel=True
    )
    graph.compile()
...
async def test_error_handling_in_repeated_nodes_async():
    ...
    with pytest.raises(ValueError):
        graph.add_repeating_edge(
            "start_task", "failing_task", END, repeat=0, parallel=True
        )
...
async def test_large_scale_parallel_performance():
    ...
    graph2.add_repeating_edge(
        "start_task_2", "cpu_intensive_task_2", END, repeat=10, parallel=False
    )
    graph2.compile()
