...
class Graph(BaseGraph):
    ...
    def _convert_execution_plan(self) -> List[Any]:
        ...
        def create_executable_node(exec_plan_item: Union[tuple, List[Any], None]) -> ExecutableNode:
        ...
        # Filter out the cycle marker so it does not trigger the ValueError.
        filtered_plan = [item for item in self.detailed_execution_path if item != "CYCLE"]
        self.execution_plan = [create_executable_node(item) for item in filtered_plan]
    ...
    def _execute(self, start_from: Optional[str] = None, timeout: Union[int, float] = 60 * 5) -> None:
        ...
        def extract_tasks_from_node(
            node: ExecutableNode, tasks: Optional[Union[List, Tuple]] = None
        ) -> Union[List, Tuple]:
            """
